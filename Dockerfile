...
RUN curl --silent --location http://mirror.nbtelecom.com.br/apache/httpd/httpd-$HTTPD_VERSION.tar.gz | tar xz -C /app/.heroku/php
# Config
RUN mkdir -p /app/.heroku/php/etc/apache2/
...
# Install Nginx
RUN curl --silent --location https://nginx.org/download/nginx-$NGINX_VERSION.tar.gz | tar xz -C /app/.heroku/php
# Config
RUN mkdir -p /app/.heroku/php/etc/nginx/
...
# Install PHP
RUN curl --silent --location https://www.php.net/distributions/php-$PHP_VERSION.tar.gz | tar xz -C /app/.heroku/php
# Config
...
ONBUILD COPY ./tools/start-web.sh /app/user/
ONBUILD COPY composer.lock /app/user/
ONBUILD COPY composer.json /app/user/
# Export heroku bin
ENV PATH /app/user/bin:$PATH
#ENTRYPOINT tools/start-web.sh
